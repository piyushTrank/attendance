# Generated by Django 5.1.4 on 2025-01-08 12:57

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('apis', '0006_alter_myuser_gender_alter_myuser_user_type_and_more'),
    ]

    operations = [
        migrations.AddField(
            model_name='leavesmodel',
            name='dayoption',
            field=models.CharField(blank=True, choices=[('Half', 'Half'), ('Full', 'Full')], max_length=20, null=True, verbose_name='Day Option'),
        ),
        migrations.AlterField(
            model_name='myuser',
            name='gender',
            field=models.CharField(blank=True, choices=[('Female', 'Female'), ('Male', 'Male'), ('Others', 'Others')], max_length=20, verbose_name='Gender'),
        ),
    ]
